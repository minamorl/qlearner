...
def calcrate_rcp_rmsd(dataset, relative_count):
    ...
    for t in range(dataset.shape[0]):
        ...
        mu = 1.0 / relative_count * sum(dataset[t] for _ in range(t - relative_count + 1, t))
        sigma = 1.0 / relative_count * sum(dataset[t] ** 2 - mu ** 2 for _ in range(t - relative_count+1, t))
        rcp = (dataset[t] - mu) / (4 * sigma)

        # Calcuration of RMSD (relative moving standard d)
        mu2 = 1.0 / relative_count * sum(sigma for _ in range(t - relative_count + 1, t))
        sigma2 = 1.0 / relative_count * sum(sigma ** 2 for _ in range(t - relative_count + 1)) - mu2 ** 2
